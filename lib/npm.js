...
exports.packages = function *(username) {
  ...
  var packages = [];
  for (var i = 0; i < $('.bullet-free.collaborated-packages > li > a').length; i++) {
    packages.push($('.bullet-free.collaborated-packages > li > a')[i].children[0].data);
  }
  return packages;
};
...
exports.downloads = function *(pkg, from, to) {
  ...
  data.downloads = fill(trim(data.downloads, start, end));
  return data;
};
...
function trim(arr, start, end) {
  if (moment(arr[arr.length - 1].day).isAfter(moment(end))) return trim(_.dropRight(arr), start, end);
  if (!moment(arr[arr.length - 1].day).isSame(moment(end))) arr.push({ day: end, downloads: 0 });
  if (moment(arr[0].day).isBefore(moment(start))) return trim(_.drop(arr), start, end);
  if (moment(arr[0].day).isSame(moment(start))) return arr;
