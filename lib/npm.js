...
exports.downloads = function *(pkg, from, to) {
  ...
  var start = moment.isDate(from) ? from : moment().subtract(1, 'month').format('YYYY-MM-DD');
  var end = moment.isDate(to) ? to : moment().subtract(1, 'day').format('YYYY-MM-DD');
  data.downloads = normalize(data.downloads, start, end);
  return data;
};
